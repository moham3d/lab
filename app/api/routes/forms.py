...
async def create_check_eval(
    ...
    result = await db.execute(
        select(NursingAssessment).join(FormSubmission).where(FormSubmission.visit_id == form_data.visit_id)
    )
    existing = result.scalar_one_or_none()
    if existing:
        raise HTTPException(status_code=409, detail="Nursing assessment already exists for this visit")
    ...
    assessment_data = form_data.dict(exclude={'visit_id'})
...
@router.put("/check-eval/{assessment_id}", response_model=CheckEvalResponse)
async def update_check_eval(
    ...
    result = await db.execute(select(NursingAssessment).where(NursingAssessment.assessment_id == assessment_id))
    form = result.scalar_one_or_none()
    if not form:
        raise HTTPException(status_code=404, detail="Check-Eval form not found")
...
async def create_general_sheet(
    ...
    # Check if assessment already exists
    result = await db.execute(
        select(RadiologyAssessment).join(FormSubmission).where(FormSubmission.visit_id == form_data.visit_id)
    )
    existing = result.scalar_one_or_none()
    if existing:
        raise HTTPException(status_code=409, detail="Radiology assessment already exists for this visit")
...
@router.put("/general-sheet/{assessment_id}", response_model=GeneralSheetResponse)
async def update_general_sheet(
    ...
    result = await db.execute(select(RadiologyAssessment).where(RadiologyAssessment.radiology_id == assessment_id))
    form = result.scalar_one_or_none()
    if not form:
        raise HTTPException(status_code=404, detail="General Sheet form not found")
